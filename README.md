...
At a mimimum the user should have a conceptual understanding of
the unix command line, unix filesystem, and secure shell.

Intro
---------------------------------------

Traditional bioinformatic workflows are impelemented using
either scripting (e.g. bash, phython, etc.) or a GUI program
...
hand. However, scripting or executing a workflow is still
difficult since it requires the developer to imperitively
code the workflow from start to finish since iteratively
developing such a pipeline creates an organization
bottleneck. For larger datasets, scripting is further
complicated by the need to use high performance computing
infrastructure.

We were able to simplify the constructing of custom
bioinformatic workflows by keeping track of input files 
and output files of each step of the analysis to implicitely
...
* _Reproducibity_, Make ensures that the workflows output
		is actually a representive by the most up-to-date code.
		In addition, make file supports start-to-finish execution
		of the complete workflow.
* _Simplicity_, Make determines the workflows execution order 
		on it's own, meaning that the developer does not need to
		worry about complictaed loops.
* _Efficenticy_, Make only performs computations that are
...
bioinformatic pipelines. Sometimes using Make is going
to be a bit awkward.
...
: A dependency is an input file needed to construct a target.
	Dependencies can, themselves, be targets.
Target
: A target is a file generated by a rule.
Command
: The shell command used to construct the target. Commands must
be offset with a tab.
Rule
: A rule is a single step in a workflow. It consists of a list of 
dependencies, a target, and a command. 
Makefile
: A file named `Makefile` that stores multiple rules. Makefiles
	must be saved as `makefile` or `Makefile`.
Make Directory
: The directory where the Makefile lives. The `make` command can
only be called from this directory.
...
# n.b. Hello and World are dependencies here.
HelloWorld : Hello World ;
	cat Hello World > HelloWorld

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The target Hello can now be consructed with following command:
...
The target `HelloWorld` can be constucted by the command:
...
Make is able to declatively encode a bioinformatic workflow quite
...
Make is powerful and flexible. From expirence, it seems that make
work's best when you follow the n commandments:
...
By using a schedulerer such as `slurm` or a remote access tool 
such as `ssh` we are able to outsource work to other computers.

Some very expensive proprietery schedulers are even able to use makefiles as
...
_But on an HPC cluster with a shared filesystem both my_time
and time will appear in your current directory._ The first 
command is significantly less efficent since the data `date` produced gets
...
data. In high throughput situations, the first example is disatorous and has 
...
the labs webserver). This is useful for less computational intense 
work that requires software that is difficult to get working on a 
HPC cluser; e.g., graphing and gene set analysis software. Note that,
you need to mount the remote filesystem on your local coputer or
you must move input files need to be moved to the remote server
explictly and output files need to be moved back. This is easier
said than done. It's probably simplest to keep two make files:
one Makefile for high throughput analysis and second Makefire for
the analysis that can take place on a personal computer.

### Organizign the expirement #########

Another potential caveat of the Makefile approach is that
it requires you to organize your expirements data files in a
way that is---well---very organized. See the example Makefile
which goes in depth for a generic RNAseq design that analyses
multiple expirements in parallel.

In short, files need to be predictable places. As you move down
the directory tree you should move from general files to specific
files.

But as a rule of thumb, if you're Makefile has a lot of expirement
specefic rules or a lot of funky string manipulation going on, chances
...
#TODO Replace with working Makefile
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
